...
# 今回のタスク: Step 1 — core（enums / types / errors / clock）

## 最初に必ず読むファイル

1. AGENTS.md                      … 実装ルール（最優先）
2. docs/implementation-plan.md    … 実装順序とテスト項目。**§3 の「Step 1」の節が今回の仕様**
...
docs/implementation-plan.md §3 の **Step 1** の節に書かれている内容を、そのとおりに実装する。

作成対象ファイル（目安。正確な一覧は implementation-plan.md の該当節を見ること）:
src/deskmate/__init__.py, src/deskmate/core/__init__.py, core/enums.py, core/types.py, core/errors.py, core/clock.py

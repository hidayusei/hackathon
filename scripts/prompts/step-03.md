...
# 今回のタスク: Step 3 — input（base / scenarios / dummy / factory）

## 最初に必ず読むファイル

1. AGENTS.md                      … 実装ルール（最優先）
2. docs/implementation-plan.md    … 実装順序とテスト項目。**§3 の「Step 3」の節が今回の仕様**
...
docs/implementation-plan.md §3 の **Step 3** の節に書かれている内容を、そのとおりに実装する。

作成対象ファイル（目安。正確な一覧は implementation-plan.md の該当節を見ること）:
src/deskmate/input/__init__.py, input/base.py, input/scenarios.py, input/dummy_source.py, input/factory.py, tests/test_dummy_source.py

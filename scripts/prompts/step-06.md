...
# 今回のタスク: Step 6 — runner（スレッド統合・headless 動作確認）

## 最初に必ず読むファイル

1. AGENTS.md                      … 実装ルール（最優先）
2. docs/implementation-plan.md    … 実装順序とテスト項目。**§3 の「Step 6」の節が今回の仕様**
...
docs/implementation-plan.md §3 の **Step 6** の節に書かれている内容を、そのとおりに実装する。

作成対象ファイル（目安。正確な一覧は implementation-plan.md の該当節を見ること）:
pipeline/runner.py, src/deskmate/app.py, src/deskmate/__main__.py, src/deskmate/logging_setup.py（簡易版）, tests/test_runner.py

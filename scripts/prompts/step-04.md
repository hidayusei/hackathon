...
# 今回のタスク: Step 4 — pipeline 前半（windower / regions / features / history）

## 最初に必ず読むファイル

1. AGENTS.md                      … 実装ルール（最優先）
2. docs/implementation-plan.md    … 実装順序とテスト項目。**§3 の「Step 4」の節が今回の仕様**
...
docs/implementation-plan.md §3 の **Step 4** の節に書かれている内容を、そのとおりに実装する。

作成対象ファイル（目安。正確な一覧は implementation-plan.md の該当節を見ること）:
src/deskmate/pipeline/__init__.py, pipeline/windower.py, pipeline/regions.py, pipeline/features.py, pipeline/history.py, tests/test_windower.py, tests/test_regions.py, tests/test_features.py, tests/test_history.py

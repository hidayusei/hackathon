...
| 担当 | 役割 |
|------|------|
| Claude Code | 設計・レビュー担当。`docs/` 配下の設計書を作成・更新する |
| **Codex（あなた）** | **実装担当。設計書に従って `src/` `tests/` `config/` を実装する** |
...
`docs/implementation-plan.md` §2 の Step 0 → 14 の順に実装する。**順序を飛ばさない。**
各 Step の「完了条件」を満たしてから次へ進む。
...
- `QTimer` の interval は設定から取る（`ui.detail.refresh_hz`, `character.fps` など）。
...
3. **`ShareWindow` から `FeatureFrame` / `DetailFrame` / `EventWindow` を import しない。**
   `detail_updated` シグナルにも接続しない。
...
8. **既定値を `false` から変えない。**
   ...
   `debug.enabled` / `privacy.allow_external_send`。
...
例:
feat: Step 4 特徴量抽出とグリッド集計を実装
test: Step 5 平滑化の遷移表テストを追加
fix: Step 6 空窓時のゼロ除算を修正
```
